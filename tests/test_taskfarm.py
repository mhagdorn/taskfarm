...
class TaskfarmTest(TestCase):
    ...
    def get_run_info(self,run_uuid,info=None,status=200):
        ...
        if info is not None:
            data['info'] = info
        response = self.app.get('/api/runs/'+run_uuid, headers=headers, query_string = data)
    ...
    def restart_run(self,run_uuid,all=None,status=204):
        ...
        if all is not None:
            data['all'] = all
        response = self.app.post('/api/runs/{}/restart'.format(run_uuid), headers=headers, query_string = data)
        self.assertEqual(response.status_code, status)
    ...
    def test_get_task_wrongworker(self):
        nt=10
        run = self.create_run(nt)
        task = self.get_task(run['uuid'],'no_worker',status=404)
        
    def test_get_task_norun(self):
        worker = self.create_worker('blub',1000)
        task = self.get_task('norun',worker['uuid'],status=404)
    ...
    def test_get_task(self):
        ...
        # all tasks should have status computing and there should be no waiting tasks
        rinfo = self.get_run_info(run['uuid'])
        self.assertEqual(rinfo['numComputing'],nt)
        self.assertEqual(rinfo['numWaiting'],0)
        
    def test_get_task_info_norun(self):
        tinfo = self.get_task_info('norun',1,status=404)

    def test_get_task_info_outofrange(self):
        nt=10
        run = self.create_run(nt)
        tinfo = self.get_task_info(run['uuid'],-1,status=404)
        tinfo = self.get_task_info(run['uuid'],nt,status=404)

    def test_get_task_info_wronginfo(self):
        nt=10
        run = self.create_run(nt)
        tinfo = self.get_task_info(run['uuid'],0,info='no_such_thing',status=404)
    ...
    def test_update_task_info(self):
        ...
        self.update_task_info(run['uuid'],tid,{'status':'wrong'},status=400)
    ...
    def test_restart_run(self):
        ...
        for i in range(0,nt,2):
            self.update_task_info(run['uuid'],i,{'status':'done','percentCompleted':100})
        for i in range(1,nt,2):
            self.update_task_info(run['uuid'],i,{'status':'computing','percentCompleted':50})
        rinfo = self.get_run_info(run['uuid'])
        self.assertEqual(rinfo['numWaiting'],0)
